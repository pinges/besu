...
public class PrivacyAccount {
  ...
  public static PrivacyAccount create(
          final URL privateKeyPath, final URL enclavePublicKeyPath, final URL enclavePrivateKeyPath) {
    return new PrivacyAccount(privateKeyPath, new URL[] {enclavePublicKeyPath}, new URL[] {enclavePrivateKeyPath});
  }
  ...
  public String[] getEnclaveKeyPaths() {
    return Arrays.stream(enclaveKeyPaths).map(path -> toStringResource(path)).toArray(String[]::new);
  }

  public String[] getEnclavePrivateKeyPaths() {
    return Arrays.stream(enclavePrivateKeyPaths).map(path -> toStringResource(path)).toArray(String[]::new);
  }
