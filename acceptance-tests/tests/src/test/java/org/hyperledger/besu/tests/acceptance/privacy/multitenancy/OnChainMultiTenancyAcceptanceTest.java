...
import static org.hyperledger.besu.ethereum.core.Address.ONCHAIN_PRIVACY;

public class OnChainMultiTenancyAcceptanceTest extends OnChainPrivacyAcceptanceTest {
...
//  private static final Accounts accounts = new Accounts(new EthTransactions());
...
//  @Rule public WireMockRule wireMockRule = new WireMockRule(options().dynamicPort());
  ...
  public void setUp() throws Exception {
    alice =
      ...
      aliceBesu.useAuthenticationTokenInHeaderForJsonRpc(token);
  ...
  @Test
    public void createPrivacyWithOtherEnclaveKey() {
      final BesuNode aliceBesu = alice.getBesu();
      final String token =
              aliceBesu.execute(permissioningTransactions.createSuccessfulLogin("user2", "Password2"));
      aliceBesu.useAuthenticationTokenInHeaderForJsonRpc(token);
    final String privateFrom = alice.getOrion().getPublicKeys().get(1);
    createOnChainPrivacyGroup(privateFrom, Arrays.asList(alice.getEnclaveKey(), alice.getOrion().getPublicKeys().get(1), alice.getOrion().getPublicKeys().get(2)), alice);
  }
