...
public class OrionTestHarnessFactory {
  ...
  public static OrionTestHarness create(
    ...
    final Path[] pubKeys = Arrays.stream(pubKeyPaths).map((pk) -> {
              try {
    ...
    final Path[] privKeys = Arrays.stream(privKeyPaths).map((pk) -> {
              try {
                return copyResource(pk, tempDir.resolve(pk));
              } catch (final IOException e) {
                throw new RuntimeException(e);
  ...
  public static OrionTestHarness create(
          final Path tempDir, final Path[] key1pubs, final Path[] key1key, final List<String> othernodes) {
